...
# 0 = background, 1 = boundary, 2 = filled
...
for yy in range(HEIGHT):
    for xx in range(WIDTH):
        inside[yy, xx] = polygon.contains_point((xx, yy))
...
if (0 <= seed_x < WIDTH and
    0 <= seed_y < HEIGHT and
    inside[seed_y, seed_x] and
    canvas[seed_y, seed_x] == 0):
...
while queue:
    ...
    if canvas[y, x] == 1 or canvas[y, x] == 2:
        continue
...
ax.set_title("Boundary Fill Algorithm")

def animate(i):
    img.set_array(frames[i])
    ax.set_title(f"Boundary Fill Algorithm - Step {i+1}")
    return img,
